...
train_add = pd.read_csv('./input/validation/train_add.csv'%folder)
test_add = pd.read_csv('./input/validation/test_add.csv'%folder)
...
## load other data
member = pd.read_csv('./input/validation/members_nn.csv').sort_values('msno')
song = pd.read_csv('./input/validation/songs_nn.csv').sort_values('song_id')
...
train = train.merge(member[['msno', 'city', 'gender', 'registered_via', \
        'msno_timestamp_mean', 'msno_rec_cnt', 'msno_timestamp_std', \
...
for i in range(100):
    ...
    sample_weight_rate = para['sample_weight_rate'].values[i]
    '''
    ...
    model_path = 'bst_model_%s_%d.h5'%(datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'), np.random.randint(0,65536))
    ...
    test_flow_predict = dataGenerator.flow(test_embeddings+test_genre, \
            [test_context], batch_size=16384, shuffle=False)
